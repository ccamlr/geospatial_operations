...
FCA=function(Input,Plot=NULL,PlotName=NULL,PlotTitle=NULL,Res=50000,Qs=c(0,0.25,0.5,0.75,1),TQ=0.5,smoo=1,ID=NA){
  ...
  if(nrow(Vs)==1){Vs=Vs[rep(1,2),];Vs[1,1]=0} #if only one values (all records falling in the same cell) repeat them to enable approx
  ...
  if(nrow(Pcont)==1){ #For cases where there are two few record, all falling in the same cell
    Pcont$c=hcl.colors(n=1,palette="viridis")
  }else{
