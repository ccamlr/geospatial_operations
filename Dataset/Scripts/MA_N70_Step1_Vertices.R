...
sName="MA_N70_Step1_Vertices.R"

#NB this is a special case, with a hole inside the polygon (dealt with in Step2)!
...
MVtmp=bind_rows(
  PV[which(PV$Vertex=="P1"):which(PV$Vertex=="P5"),(1:3)],
  SV[which(SV$Vertex=="S18"):which(SV$Vertex=="S19"),(1:3)],
  SV[which(SV$Vertex=="S24"),(1:3)],
  PV[which(PV$Vertex=="P6"):which(PV$Vertex=="P7"),(1:3)],
  SV[which(SV$Vertex=="S85"):which(SV$Vertex=="S87"),(1:3)],
  PV[which(PV$Vertex=="P8"):which(PV$Vertex=="P12"),(1:3)]
)
MVtmp$Name=pName
