...
#Set the name of the polygon in Step1 script

# tmp=read.csv("I:/Science/Projects/Geospatial Operations/GIS Workflow/Polygon Builder/Primary Vertices/Primary_Vertices.csv")
# tmp=tmp%>%filter(Name==pName)%>%select(Name,Latitude,Longitude)
...
# plot(st_geometry(RBs),lwd=3)
# add_labels(mode='auto',layer='RBs',fontsize=0.2)
...
PV=read.csv("I:/Science/Projects/Geospatial Operations/GIS Workflow/Polygon Builder/Primary Vertices/Primary_Vertices.csv")

PV=PV[grep("_",PV$Name),]

ttt=create_Polys(PV[,c("Name","Latitude","Longitude"),])
plot(st_geometry(ttt))



png(filename="CheckRBs.png",width=5000,height=5000,res=600)

par(mai=c(0.3,0.3,0.3,0.3),xaxs="i",yaxs="i",xpd=T)
plot(st_geometry(RBs),lwd=3,border="red")
plot(st_geometry(ttt),lwd=1,border="green",add=T)
...
PV=PV%>%filter(Name%in%c('882_1','883_2',"883_12","883_3"))


tmpP=create_Polys(PV[,c("Name","Latitude","Longitude"),])
tmpp=create_Points(PV[,c("Latitude","Longitude","Vertex"),])

png(filename="CheckRBs_P.png",width=5000,height=5000,res=600)
plot(st_geometry(tmpP))
text(tmpP$Labx,tmpP$Laby,tmpP$ID)
text(tmpp$x,tmpp$y,tmpp$Vertex,col="darkgreen")
dev.off()
...
plot(PV$Longitude,PV$Latitude,type="b")
